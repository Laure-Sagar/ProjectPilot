...
function listBoards() {
    ...
    for (let _board of appData.boards) {
        ...
        if (_board.id === currentBoard().id)
            _boardTitle.classList.add("is-active");
        ...
        if (_hoverCard === _heldItemObject.getParentCard()) {
            ...
            if (_hoverItem) {
                ...
                if (_hoverItem !== cardDrag_mouseDownOn) {
                    ...
                    getCardFromElement(
                        _heldItemObject.getParentCard()
                    ).removeItem(_heldItemObject);
...
const cardContextMenu_clearCard = () => {
    createConfirmDialog("Are you sure to clear this board", () => {
        let _currentCardObject = getCardFromElement(
            cardContextMenu_currentCard
        );
...
function loadData() {
    ...
    if (_data) {
        ...
        appData.identifier =
            _appData.identifier !== null ? appData.identifier : 0;
